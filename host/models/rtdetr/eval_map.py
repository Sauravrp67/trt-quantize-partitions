...
if __package__:                    # python -m models.rtdetr.eval_map
    from . import _bootstrap       # noqa: F401  puts repo root + this dir on sys.path
else:                              # python host/models/rtdetr/eval_map.py
...
def _write_table(rows, out_path: Path, *, spec, backend_label: str, n_images: int) -> Path:
    ...
    lines += ["", "_Accuracy only — no score threshold applied (COCOeval sweeps them). "
              "For latency see `host/models/rtdetr/infer.py` or `trtexec` with locked clocks._", ""]
...
def main() -> None:
    ...
    parser.add_argument("--precision", nargs="+", default=None,
                        help="ONNX variant names, e.g. fp32 fp16; the first is the Δ reference")
    parser.add_argument("--backend", default="ort", choices=["ort", "trt", "torch"],
                        help="ort/trt read the ONNX; torch evaluates the eager checkpoint "
                             "(the pre-export reference — ignores --precision)")
    ...
    refs = args.precision or [spec.default_onnx]
    if args.backend == "torch":
        refs = refs[:1]   # eager weights are the same regardless of --precision
    onnx_paths = [spec.onnx_path(r) for r in refs]
    if args.backend != "torch":
        for path in onnx_paths:
            require(path, "ONNX model", "build it with host/models/rtdetr/export.py")
    ...
    adapter = load_adapter(spec, onnx=refs[0], device=device)
    ...
    for ref, path in zip(refs, onnx_paths):
        ...
        backend = build_backend(args.backend, adapter, providers=providers,
                                onnx_path=str(path), tf32=tf32, timing_cache=timing_cache)
