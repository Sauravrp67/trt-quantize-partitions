...
# Off unless a caller asks for one. Where the cache lives is a spec decision rather than
# a harness constant: see `defaults.timing_cache` in configs/*.yaml, surfaced as
# `ModelSpec.timing_cache` and scoped per backbone per (GPU, TensorRT version).
DEFAULT_TIMING_CACHE = None


class EngineBuildError(RuntimeError):
    """TensorRT refused to build an engine for this graph.

    Distinct from a harness bug: some precision partitions are simply not compilable,
    so callers sweeping partitions should catch this and record the partition as
...
def build_engine(
    ...
    *,
    ...
    """Serialized engine for ``onnx_path``. Precision comes from the graph, never a flag.

    ``timing_cache`` is a path, read before the build and rewritten after it, so a tactic
    is timed once rather than on every build. Callers holding a spec should pass
    ``spec.timing_cache``; ``None`` builds without a cache.
    """
    ...
    if serialized is None:
        # TensorRT reports the cause on its own logger and returns None. Raising here keeps
        # the failure attached to the graph that caused it -- a partition can be *infeasible*
        # (e.g. FP32 nodes scattered inside a Myelin-fused region make NVRTC codegen fail),
        # which is a property of the partition worth recording, not an unexpected crash.
        raise EngineBuildError(f"TensorRT could not build an engine from {onnx_path}; "
                               f"see the TensorRT log above for the failing node")
