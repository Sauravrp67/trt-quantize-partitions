...
def resolve_engine(*, engine=None, onnx_path=None, tf32: bool = True,
                   timing_cache=None) -> bytes:
    ...
    if onnx_path is not None:
        return build_engine(str(onnx_path), tf32=tf32, timing_cache=timing_cache)
...
class TrtBackend:
    ...
    def __init__(self, adapter: DetectorAdapter, *, engine=None, onnx_path=None,
                 tf32: bool = True, timing_cache=None, label: str = "TensorRT") -> None:
...
def run_inference(adapter, source, *, engine=None, onnx_path=None, tf32: bool = True,
                  timing_cache=None, backend_label: str = "TensorRT", **kw):
    """Single-backend TensorRT run (own window/summary). See harness.runner.run_backends."""
    from .runner import run_backends
    backend = TrtBackend(adapter, engine=engine, onnx_path=onnx_path, tf32=tf32,
                         timing_cache=timing_cache, label=backend_label)
