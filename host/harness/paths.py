...
_ROOT_MARKERS = ("host/harness", "host/configs")
...
def find_repo_root(start: Optional[Path] = None) -> Path:
    ...
    for candidate in (here, *here.parents):
        if candidate.is_dir() and all((candidate / m).is_dir() for m in _ROOT_MARKERS):
            return candidate
    return Path(__file__).resolve().parents[2]   # installed oddly; fall back to layout
...
CONFIGS_DIR = REPO_ROOT / "host" / "configs"
MODELS_DIR = REPO_ROOT / "host" / "models"
HARNESS_DIR = REPO_ROOT / "host" / "harness"
PIPELINE_DIR = REPO_ROOT / "host" / "pipeline"
